...
const BentoCard = ({
  ...
  <div
    ...
    className={cn(
      ...
      // light styles
      "bg-background [box-shadow:0_0_0_1px_rgba(0,0,0,.05),0_2px_4px_rgba(0,0,0,.06),0_12px_24px_rgba(0,0,0,.06)]",
      // dark styles
  ...
  >
    ...
    <div className="p-4">
      <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 transition-all duration-300 lg:group-hover:-translate-y-8">
        ...
        <p className="max-w-lg text-neutral-700 dark:text-neutral-300">{description}</p>
      </div>
    ...
    <div
      className={cn(
        "hidden lg:flex pointer-events-none absolute bottom-0 w-full translate-y-10 transform-gpu flex-row items-center px-2 py-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100",
      )}
    >
      <Button
        variant="link"
        asChild
        size="sm"
        className="pointer-events-auto p-0 text-neutral-100"
      >
        <a href={href}>
          {cta}
          <ArrowRightIcon className="ms-2 h-4 w-4 rtl:rotate-180" />
    ...
    <div className="pointer-events-none absolute inset-0 transform-gpu transition-all duration-300 group-hover:bg-black/[.04] group-hover:dark:bg-neutral-800/20" />
  </div>
);
